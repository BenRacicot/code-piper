      ...
      .filter(
        (s) =>
          ...
          (s.filepath !== currentFilepath &&
            // Exclude CodePiper's own output as it makes it super-hard for users to test the autocomplete feature
            // while looking at the prompts in the CodePiper output
            !s.filepath.startsWith(
              "output:extension-output-ModelPiper.codepiper",
            )),
      )
      .sort((a, b) => b.timestamp - a.timestamp)
