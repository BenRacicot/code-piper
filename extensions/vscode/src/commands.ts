...
function getFullScreenTab() {
  ...
  return tabs.find((tab) =>
    (tab.input as any)?.viewType?.endsWith("codepiper.codePiperGUIView"),
  );
}
...
function focusGUI() {
  ...
  if (fullScreenTab) {
    ...
    // focus sidebar
    vscode.commands.executeCommand("codepiper.codePiperGUIView.focus");
...
const getCommandsMap: (
  ...
  return {
    "codepiper.acceptDiff": async (newFileUri?: string, streamId?: string) => {
    ...
    "codepiper.rejectDiff": async (newFileUri?: string, streamId?: string) => {
    ...
    "codepiper.defaultQuickAction": async (args: QuickEditShowParams) => {
      captureCommandTelemetry("defaultQuickAction");
      vscode.commands.executeCommand("codepiper.focusEdit", args);
    },
    "codepiper.customQuickActionSendToChat": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "codepiper.customQuickActionStreamInlineEdit": async (
      prompt: string,
      range: vscode.Range,
    ) => {
    ...
    "codepiper.codebaseForceReIndex": async () => {
      core.invoke("index/forceReIndex", undefined);
    },
    "codepiper.rebuildCodebaseIndex": async () => {
      core.invoke("index/forceReIndex", { shouldClearIndexes: true });
    },
    "codepiper.docsIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: false });
    },
    "codepiper.docsReIndex": async () => {
      core.invoke("context/indexDocs", { reIndex: true });
    },
    "codepiper.focusCodePiperInput": async () => {
      const isCodePiperInputFocused = await sidebar.webviewProtocol.request(
        "isCodePiperInputFocused",
        undefined,
        false,
      );
      ...
      if (isCodePiperInputFocused) {
        if (historyLength === 0) {
          hideGUI();
        } else {
          void sidebar.webviewProtocol?.request(
            "focusCodePiperInputWithNewSession",
            undefined,
            false,
          );
        }
    ...
    "codepiper.focusCodePiperInputWithoutClear": async () => {
      const isCodePiperInputFocused = await sidebar.webviewProtocol.request(
        "isCodePiperInputFocused",
        undefined,
        false,
      ...
      if (isCodePiperInputFocused) {
        ...
        sidebar.webviewProtocol?.request(
          "focusCodePiperInputWithoutClear",
          undefined,
        );
    ...
    "codepiper.writeDocstringForCode": async () => {
    ...
    "codepiper.optimizeCode": async () => {
      captureCommandTelemetry("optimizeCode");
      streamInlineEdit("optimize", "Optimize this code");
    },
    "codepiper.fixGrammar": async () => {
    ...
    "codepiper.clearConsole": async () => {
      consoleView.clearLog();
    },
    "codepiper.viewLogs": async () => {
      captureCommandTelemetry("viewLogs");
      vscode.commands.executeCommand("workbench.action.toggleDevTools");
    },
    "codepiper.debugTerminal": async () => {
    ...
    "codepiper.newSession": () => {
      sidebar.webviewProtocol?.request("newSession", undefined);
    },

    "codepiper.shareSession": async (sessionId: string | undefined) => {
      if (!sessionId) {
    ...
    "codepiper.viewHistory": () => {
      vscode.commands.executeCommand("codepiper.navigateTo", "/history", true);
    },
    "codepiper.focusCodePiperSessionId": async (
      ...
      void sidebar.webviewProtocol?.request("focusCodePiperSessionId", {
        sessionId,
      });
    },
    "codepiper.applyCodeFromChat": () => {
      void sidebar.webviewProtocol.request("applyCodeFromChat", undefined);
    },
    "codepiper.openConfigPage": () => {
      vscode.commands.executeCommand("codepiper.navigateTo", "/config", false);
    },
    "codepiper.selectFilesAsContext": async (
      firstUri: vscode.Uri,
      uris: vscode.Uri[],
    ) => {
      if (uris === undefined) {
    ...
    "codepiper.forceAutocomplete": async () => {
    ...
    "codepiper.openTabAutocompleteConfigMenu": async () => {
      ...
      quickPick.onDidAccept(() => {
        ...
        if (targetStatus !== undefined) {
          ...
          vscode.commands.executeCommand("codepiper.focusCodePiperInput");
        } else if (selectedOption === "$(screen-full) Open full screen chat") {
          vscode.commands.executeCommand("codepiper.openInNewWindow");
        } else if (selectedOption === "$(gear) Open settings") {
          vscode.commands.executeCommand("codepiper.navigateTo", "/config");
        }
    ...
    "codepiper.navigateTo": (path: string, toggle: boolean) => {
      sidebar.webviewProtocol?.request("navigateTo", { path, toggle });
      focusGUI();
    },
    "codepiper.startLocalOllama": () => {
      startLocalOllama(ide);
    },
    "codepiper.startLocalLemonade": () => {
      startLocalLemonade(ide);
    },
    "codepiper.installModel": async (
      modelName: string,
    ...
    "codepiper.convertConfigJsonToConfigYaml": async () => {
    ...
    "codepiper.enterEnterpriseLicenseKey": async () => {
    ...
    "codepiper.toggleNextEditEnabled": async () => {
    ...
    "codepiper.openInNewWindow": async () => {
      focusGUI();
      ...
      vscode.commands.executeCommand("codepiper.newSession");
      ...
      const sessionLoader = panel.onDidChangeViewState(() => {
        ...
        if (sessionId) {
          vscode.commands.executeCommand(
            "codepiper.focusCodePiperSessionId",
            sessionId,
          );
        }
        panel.reveal();
