...
function getSessionDir(): string {
  ...
  if (process.env.CONTINUE_CLI_TEST && process.env.HOME) {
    const sessionDir = path.join(process.env.HOME, ".codepiper", "sessions");
  ...
  // Use CODEPIPER_GLOBAL_DIR if set (for testing)
  const continueHome =
    process.env.CODEPIPER_GLOBAL_DIR || path.join(os.homedir(), ".codepiper");
