...
As with [@Codebase context provider configuration](./deprecated-codebase), you can adjust the reranking behavior of the `@Docs` context provider with the `nRetrieve`, `nFinal`, and `useReranking`.
...
<Info>
  ...
  to `~/.codepiper/.utils`, and use it as the local crawler. **Note:** Chromium
