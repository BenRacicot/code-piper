...
Create a `config.yaml` file in your Continue Global Directory (`~/.codepiper` on Mac, `%USERPROFILE%\.codepiper`) alongside your current config.json file. If a `config.yaml` file is present, it will be loaded instead of config.json.
