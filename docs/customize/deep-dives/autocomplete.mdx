...
<Tabs>
  <Tab title="Hub">
  [Mistral Codestral model block](https://continue.dev/mistral/codestral)
  </Tab>
  <Tab title="YAML">
...
<Tabs>
  <Tab title="Hub">
  [Ollama Qwen 2.5 Coder 1.5B model block](https://continue.dev/ollama/qwen2.5-coder-1.5b)
  </Tab>
  <Tab title="YAML">
  ```yaml title="config.yaml"
  name: My Config
...
Explore autocomplete model configurations on [the hub](https://continue.dev/explore/models?roles=autocomplete)
...
5. Check the output of the logs to find any potential errors: <kbd>cmd/ctrl</kbd> + <kbd>shift</kbd> + <kbd>P</kbd> -> "Toggle Developer Tools" -> "Console" tab in VS Code, ~/.codepiper/logs/core.log in JetBrains.
