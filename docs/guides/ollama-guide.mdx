...
The easiest way is to use [pre-configured model blocks](/reference#models) from the Continue Mission Control in your local configuration:

```yaml title="~/.codepiper/configs/config.yaml"
...
```yaml title="~/.codepiper/config.yaml"
models:
  - name: Autodetect
    provider: ollama
    model: AUTODETECT
