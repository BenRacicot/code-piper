...
- **Instant Setup**: Browse the [configuration marketplace](https://continue.dev) and add any config to your account with a single click
...
3. **Secure Your Keys**: Add API keys as [User Secrets](https://continue.dev/settings/secrets) in Mission Control—they're encrypted and never exposed
...
All Hub config management happens through [Mission Control](https://continue.dev). For detailed customization, see our guide on [Editing a Config](/mission-control/configs/edit-a-config).
...
- macOS/Linux: `~/.codepiper/config.yaml`
- Windows: `%USERPROFILE%\.codepiper\config.yaml`
