...
3. **Default** — your first assistant from [Continue](https://continue.dev), or `~/.codepiper/config.yaml` if you're not logged in
...
If you're not logged in to Continue, `cn` looks for `~/.codepiper/config.yaml`. This file uses the same format as the IDE extensions — see [config.yaml reference](/customize/deep-dives/configuration) for the full schema.
