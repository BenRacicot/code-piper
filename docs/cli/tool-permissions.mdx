...
Persistent permissions are stored in `~/.codepiper/permissions.yaml`. This file is updated when you choose "Continue + don't ask again" in the TUI approval prompt.

```yaml
# ~/.codepiper/permissions.yaml
allow:
  - Read(*)
  - Write(**/*.ts)
